...
export function RatingPanel({ offerId }: { offerId: string }) {
  ...
  return (
    <div className="card mt-4 p-4">
      <h3 className="text-sm font-bold text-slate-800">{t('rating.title')}</h3>
      <p className="mt-1 text-xs text-slate-500">
        {t('rating.reputation')} <strong>{ctx.partner.name}</strong> {t('rating.visible')}
      </p>
      <div className="mt-2">
      ...
      {ctx.myRating ? (
        <div className="mt-4 rounded-lg bg-slate-50 p-3 text-sm">
          {ctx.myRating.type === 'NO_RESPONSE' ? (
            <p className="font-semibold text-amber-700">{t('rating.markedNoResponse')}</p>
          ) : (
            <>
              <p className="font-semibold">{t('rating.yourRating')}</p>
          ...
          {ctx.canReview && (
            <>
              <p className="text-xs font-semibold text-slate-600">
                {ctx.myRole === 'buyer' ? t('rating.howSeller') : t('rating.howBuyer')}
              </p>
